...
const ProductTable = ({ products, onEdit, onDelete, onPrintBarcode, loading }) => {
  ...
  const getStockStatus = (stock) => {
    ...
    if (stock <= 10) return { label: 'Low Stock', class: 'bg-amber-100 text-amber-800' };
        ...
        <table className="w-full">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Prices (LKR)</th>
                  ...
                  <tr key={product.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <code className="text-xs font-mono bg-gray-100 px-2 py-1 rounded">{product.barcode}</code>
                        <button
